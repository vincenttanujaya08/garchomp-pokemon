class GarchompSummonAnimator {
  constructor({
    ...
    this.garchompScaleNode = garchompScaleNode;
    this.garchompLiftNode = garchompLiftNode;
    this.garchompOffsetNode = garchompOffsetNode || null;

    this._topClosedMatrix = this.pokeballTopNode
      ? mat4.clone(this.pokeballTopNode.localTransform)
      : null;
    this._scaleBaseMatrix = mat4.clone(this.garchompScaleNode.localTransform);
    this._liftBaseMatrix = mat4.clone(this.garchompLiftNode.localTransform);
    this._offsetBaseMatrix = this.garchompOffsetNode
      ? mat4.clone(this.garchompOffsetNode.localTransform)
      : null;

    this._offsetStart = Array.isArray(this.config.garchompOffsetStart)
      ? this.config.garchompOffsetStart
      : [0, 0, 0];
    this._offsetTarget = Array.isArray(this.config.garchompOffsetTarget)
      ? this.config.garchompOffsetTarget
      : this._offsetStart;
  ...
  update(dt) {
    ...
    switch (this.state) {
      ...
      default:
        ...
        this._setGarchompScale(this.config.finalScale);
        this._setGarchompLift(this.config.finalLift);
        this._setGarchompOffset(1);
        this._setPokeballMotion(1);
  ...
  _setGarchompScale(scale) {
    mat4.copy(this.garchompScaleNode.localTransform, this._scaleBaseMatrix);
    mat4.scale(
      this.garchompScaleNode.localTransform,
      this.garchompScaleNode.localTransform,
      [scale, scale, scale]
    );
  }

  _setGarchompLift(lift) {
    mat4.copy(this.garchompLiftNode.localTransform, this._liftBaseMatrix);
    mat4.translate(
      this.garchompLiftNode.localTransform,
      this.garchompLiftNode.localTransform,
      [0, lift, 0]
    );
  }

  _setGarchompOffset(amount) {
    if (!this.garchompOffsetNode || !this._offsetBaseMatrix) return;
